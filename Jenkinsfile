                ...
                withCredentials([string(credentialsId: 'GITHUB_PAT', variable: 'GITHUB_TOKEN')]) {
                    sshagent(credentials: [UAT_SSH_KEY]) {
                        sh '''
                            ...
                            git config --global credential.helper '!'\\''echo username=${GITHUB_TOKEN}; echo password=;\\''
                            echo "\n" | git pull https://${GITHUB_TOKEN}:@github.com/algonox/ACE-Camunda-DevOps.git ${TARGET_BRANCH}
                            git config --global --unset credential.helper # clean up credential helper
